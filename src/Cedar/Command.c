...
void CheckThread1(THREAD *thread, void *param)
{
	...
	for (i = 0;i < num;i++)
	{
		...
		InputToNull((void *)UINT32_TO_POINTER(check_thread_global_1));
...
bool CheckMemory()
{
	...
	for (i = 0;i < num;i++)
	{
		...
		{
			InputToNull(UINT32_TO_POINTER((UINT)(((UCHAR *)pp[i])[j] = j % 256)));
		}
	}
	for (i = 0;i < num;i++)
	{
...
LIST *StrToPortList(char *str)
{
	...
	for (i = 0;i < t->NumTokens;i++)
	{
		...
		if (IsInList(o, (void *)UINT32_TO_POINTER(n)))
		{
			ReleaseList(o);
			FreeToken(t);
			return NULL;
		}
		Add(o, (void *)UINT32_TO_POINTER(n));
	}
...
UINT PsClusterSettingMember(CONSOLE *c, char *cmd_name, wchar_t *str, void *param)
{
	...
	if (ParseHostPort(host_and_port, &host, &port, 0))
	{
		...
		{
			t.Ports[i] = POINTER_TO_UINT32(LIST_DATA(ports, i));
		}
...
bool CmdEvalHostAndPort(CONSOLE *c, wchar_t *str, void *param)
{
	...
	ret = ParseHostPort(tmp, NULL, NULL, POINTER_TO_UINT32(param));
